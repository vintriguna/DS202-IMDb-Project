...
imdbPopular %>% head(10) %>% mutate(Actor = fct_reorder(Actor,m)) %>% ggplot(aes(x = Actor, y = m, fill = Actor)) + geom_bar(stat = "identity") + coord_flip() + xlab("Avg IMDb popularity")


#Potential limitation: We are filtering by series actors, such as Star Wars, LOTR, etc., actors who make this list despite not being in a series might be considered extraordinarily good

#We take a look at the most common entries in this list, regardless of their avg. score
imdb2 %>% group_by(Actor) %>% summarise(m = mean(IMDB_Rating), sample_size = n()) %>% filter(sample_size >= 4) %>% arrange(desc(sample_size))
...
mostCommon %>% head(10) %>% mutate(Actor = fct_reorder(Actor,sample_size)) %>% ggplot(aes(x = Actor, y = sample_size, fill = Actor)) + geom_bar(stat = "identity") + coord_flip() + ylab("Avg IMDb popularity") + theme_bw() #+ scale_fill_brewer("Set3")
...
imdbPopular %>% head(10) %>% mutate(Actor = fct_reorder(Actor,m)) %>% ggplot(aes(x = Actor, y = m, fill = Actor)) + geom_bar(stat = "identity") + coord_flip() + ylab("Avg IMDb popularity") + theme_bw() #+ scale_fill_brewer("Set3")
```


**Actor popularity across the ages ** 
...
# Comparing metascore to IMDb score
